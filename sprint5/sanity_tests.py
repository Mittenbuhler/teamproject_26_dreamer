...
#ausführen mit pytest sprint5/sanity_tests.py 
...
def test_replay_buffer():
    ...
    # Buffer darf nicht größer als Capacity werden
    assert len(rb) == 2
...
def test_rssm_forward_shapes():
    ...
    observations = torch.randn(4, 17, 2)
    ...
    assert out["prior_predictions"]["observation"].shape == (4, 16, 2)
...
def test_imagination_shapes():
    ...
    assert imag["features"].shape == (
        3,
        10,
        model.stoch_size + model.h_size,
    )
...
def test_world_model_loss_is_finite():
    ...
    observations = torch.randn(2, 17, 2)
...
def test_gradients_exist():
    ...
    loss, _ = compute_world_model_loss(
        out,
        observations,
        rewards,
        continues,
